...
export const TableListProgram = () => {
  ...
  const { mutate: deleteProgram, isPending: onDeletingProgram } =
    useDeleteProgram();

  const [searchQuery, setSearchQuery] = React.useState("");
  const [showDeleteModal, setShowDeleteModal] = React.useState(false);
  const [programIdToDelete, setProgramIdToDelete] = React.useState<
    number | string
  >(0);
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500 ml-3">
            Manage academic catalogs and university offerings
          </p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-2xl flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium text-gray-700">
              Syncing deletion with production...
            </p>
          </div>
        </div>
      )}
                ...
                paginatedData.map((program) => (
                  ...
                  >
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 rounded-lg shadow-sm">
                          <AvatarFallback
                            className={`rounded-lg bg-gradient-to-br ${getAvatarGradient(program.id)} text-white text-xs font-bold`}
                          >
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className="text-[10px] uppercase font-bold text-indigo-600 bg-indigo-50 border-indigo-100"
                      >
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 animate-in zoom-in duration-200">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">
                This will permanently remove the program from the production
