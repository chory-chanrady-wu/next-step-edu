...
const FormLabel = ({ children, required, info }: { children: React.ReactNode; required?: boolean; info?: string }) => (
        ...
        {info && (
            <div className="group relative">
                ...
                <div className="absolute bottom-full right-0 mb-3 w-56 p-3 bg-gray-900 text-white text-[10px] rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-2xl leading-relaxed">
                    <div className="font-black mb-1 uppercase tracking-widest text-blue-400">Pro Tip</div>
                    {info}
                </div>
            </div>
        )}
...
const UserTextInput = ({ error, icon: Icon, className, showToggle, ...props }: any) => {
    ...
    return (
        <div className="space-y-2">
            <div className="relative group">
                {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-gray-400 group-focus-within:text-blue-500 transition-colors" />}
                <input
                    {...props}
                    type={type}
                    className={cn(
...
const UserForm = ({ initialData, mode = "create" }: UserFormProps) => {
    ...
    const validateForm = () => {
        ...
        else if (formData.password && formData.password.length < 8) newErrors.password = "Min. 8 characters required";
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] animate-in zoom-in duration-500">
                ...
                <p className="text-gray-500 text-center max-w-sm font-medium leading-relaxed">
                    The membership record for <span className="text-blue-600 font-bold">"{formData.full_name}"</span> has been successfully indexed into our secure database.
                </p>
            </div>
        );
    }
                ...
                <div className="lg:col-span-4 space-y-8">
                    <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white rounded-[2.5rem] overflow-hidden">
                        <CardContent className="p-8 flex flex-col items-center">
                            ...
                            >
                                <div className="w-32 h-32 rounded-[2rem] bg-gradient-to-br from-gray-50 to-blue-50 border-2 border-white shadow-xl overflow-hidden flex items-center justify-center transition-all group-hover:scale-105 duration-300">
                                    {formData.avatar_url ? (
                            ...
                            <div className="text-center space-y-1">
                                <h3 className="font-black text-gray-900 font-outfit uppercase tracking-wider">{formData.full_name || "New Member"}</h3>
                                <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.15em] bg-blue-50 px-3 py-1 rounded-full">{formData.role}</p>
                            </div>

                            <div className="w-full mt-8 pt-8 border-t border-gray-50 space-y-4">
                                <div className="p-4 bg-gray-50/50 rounded-2xl">
                                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">Member Group</p>
                                        ...
                                        {['student', 'recruiter', 'admin'].map(r => (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    formData.role === r ? "bg-blue-600 text-white shadow-md shadow-blue-500/20" : "bg-white text-gray-400 hover:text-gray-600 border border-gray-100"
                                                )}
                                            >
                                                {r[0]}
                                            </button>
                    ...
                    <div className="p-6 bg-gradient-to-br from-indigo-600 to-blue-700 rounded-[2rem] text-white shadow-2xl shadow-blue-500/20">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h4 className="font-black font-outfit text-lg">Identity Security</h4>
                        </div>
                        <p className="text-xs text-blue-100/70 font-medium leading-relaxed mb-6"> Ensure all identification details match official legal documents to maintain workspace integrity.</p>
                    ...
                    <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white rounded-[2.5rem]">
                        <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-xl font-black text-gray-900 font-outfit uppercase tracking-tight">Account Essentials</h3>
                            </div>
                        </div>
                        <CardContent className="p-10 space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-1">
                                    <FormLabel required info="Legal name as it should appear across the organization.">Full Profile Name</FormLabel>
                                    <UserTextInput
                                        name="full_name"
                                ...
                                <div className="space-y-1">
                                    <FormLabel info="Personal or work contact number with country code.">Contact Number</FormLabel>
                                    <UserTextInput
                                        name="phone"
                            ...
                            <div className="space-y-1">
                                <FormLabel required info="Unique identifier for authentication/communications.">Account Email Address</FormLabel>
                                <UserTextInput
                                    name="email"
                            ...
                            <div className="space-y-1">
                                <FormLabel required={mode === "create"} info="Access credentials must be at least 8 characters long.">Sensitive Access Password</FormLabel>
                                <UserTextInput
                                    name="password"
                                    value={formData.password}
                                    onChange={handleChange}
                                    placeholder={mode === "create" ? "••••••••" : "Leave blank to keep existing"}
                                    ...
                                    {['active', 'inactive'].map(s => (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                formData.status === s ? "bg-white text-blue-600 shadow-sm" : "text-gray-400 hover:text-gray-600"
                                            )}
                                        >
                                            {s}
                                        </button>
