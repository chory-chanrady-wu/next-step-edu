...
import { useAllProfiles, useDeleteProfile, useUpdateUserStatus } from "@/hooks/use-queries-hook";
...
const UserTable = () => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return profilesList.filter((user: UserProfileResponse) => {
      ...
      const userRoleLower = (user as any)?.role ? String((user as any).role).toLowerCase() : "";
  ...
  const renderRoleBadge = (user: UserProfileResponse) => {
    const role = (user as any)?.role ? String((user as any).role).toUpperCase() : "USER";
          ...
          <div className="flex gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40 rounded-xl">
                ...
                <DropdownMenuItem onClick={() => { setRoleFilter("user"); setCurrentPage(1); }}>Student</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="h-10 gap-2 text-gray-600 rounded-xl border-gray-200">
                  ...
                  {statusFilter === "all" ? "All Status" : statusFilter === "active" ? "Active" : statusFilter === "inactive" ? "Inactive" : "Suspended"}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-40 rounded-xl">
                <DropdownMenuItem onClick={() => { setStatusFilter("all"); setCurrentPage(1); }}>All Status</DropdownMenuItem>
                <DropdownMenuItem onClick={() => { setStatusFilter("active"); setCurrentPage(1); }} className="text-green-600">
                  Active
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => { setStatusFilter("inactive"); setCurrentPage(1); }} className="text-amber-600">
                  Inactive
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => { setStatusFilter("suspended"); setCurrentPage(1); }} className="text-red-600">
                  Suspended
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell colSpan={6} className="h-40 text-center text-gray-500 font-medium font-outfit">
              ...
              <TableRow>
                <TableCell colSpan={6} className="h-40 text-center text-red-500 font-medium font-outfit">
                  <div className="flex flex-col items-center justify-center gap-2">
                    <p>Failed to load users.</p>
                    <p className="text-xs text-gray-400">{(error as any)?.message}</p>
                    <Button
                      variant="outline"
                      size="sm"
                  ...
                  <TableRow key={user.id} className="hover:bg-blue-50/10 transition-colors border-gray-50">
                    ...
                    <TableCell>
                      <Badge
                        className={cn(
                          "font-bold text-[10px] uppercase tracking-widest px-2.5 py-0.5 border-none rounded-lg",
                          (user.status || "ACTIVE").toUpperCase() === "ACTIVE" && "bg-green-100 text-green-700",
                          (user.status || "").toUpperCase() === "INACTIVE" && "bg-amber-100 text-amber-700",
                    ...
                    {/* ✅ ROLE COLUMN */}
                    <TableCell>
                      <div className="flex items-center gap-2">{renderRoleBadge(user)}</div>
                    </TableCell>
                          ...
                          <DropdownMenuSub>
                            ...
                            <DropdownMenuSubContent className="w-40 rounded-xl p-1 shadow-lg border-gray-100">
                              <DropdownMenuItem
                                ...
                                disabled={isUpdatingStatus || (user.status || "ACTIVE").toUpperCase() === "ACTIVE"}
                              >
                                <ShieldCheck className="h-4 w-4 text-green-500" />
                                <span className="font-bold text-gray-700 text-[10px] uppercase">Active</span>
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() => handleStatusUpdate(user.userId, "INACTIVE")}
                                className="flex items-center gap-2 cursor-pointer py-2 rounded-lg"
                                disabled={isUpdatingStatus || (user.status || "").toUpperCase() === "INACTIVE"}
                              >
                                <Ban className="h-4 w-4 text-amber-500" />
                                <span className="font-bold text-gray-700 text-[10px] uppercase">Inactive</span>
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() => handleStatusUpdate(user.userId, "SUSPENDED")}
                                className="flex items-center gap-2 cursor-pointer py-2 rounded-lg"
                                disabled={isUpdatingStatus || (user.status || "").toUpperCase() === "SUSPENDED"}
                              >
                                <ShieldAlert className="h-4 w-4 text-red-500" />
                                <span className="font-bold text-gray-700 text-[10px] uppercase">Suspended</span>
                              </DropdownMenuItem>
                            </DropdownMenuSubContent>
                          </DropdownMenuSub>
