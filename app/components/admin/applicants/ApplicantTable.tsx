...
import { useAllApplicants, useDeleteApplicant, useUpdateApplicantStatus } from "@/hooks/use-queries-hook";
...
const ApplicantTable = () => {
    ...
    const [applicantToDelete, setApplicantToDelete] = useState<{ id: number, name: string } | null>(null);
    ...
    const handleDeleteClick = (id: number, firstName: string, lastName: string) => {
        setApplicantToDelete({ id, name: `${firstName} ${lastName}` });
    ...
    const handleStatusUpdate = (id: number, status: string) => {
        updateStatus({ id, status }, {
            onSuccess: () => toast.success(`Applicant ${status.toLowerCase()} successfully`),
            onError: (err: any) => toast.error(err.response?.data?.message || `Failed to ${status.toLowerCase()} applicant`)
        });
    };
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center p-12 bg-white rounded-xl border border-gray-100 shadow-sm">
                ...
                <span className="ml-3 text-gray-500 font-medium font-outfit">Loading applicants...</span>
            </div>
        );
    }
                ...
                <div className="flex flex-1 items-center gap-3 w-full md:max-w-md">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40 rounded-xl shadow-xl">
                            ...
                            <DropdownMenuItem onClick={() => setStatusFilter("approved")} className="cursor-pointer font-medium text-green-600">Approved</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setStatusFilter("rejected")} className="cursor-pointer font-medium text-red-600">Rejected</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
            </div>
            ...
            <div className="rounded-xl border border-gray-100 bg-white shadow-sm overflow-hidden">
                    ...
                    <TableBody>
                        {paginatedData.length > 0 ? (
                            paginatedData.map((app) => (
                                <TableRow key={app.id} className="hover:bg-blue-50/20 transition-colors border-gray-100">
                                    <TableCell className="py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col min-w-0">
                                                <span className="text-sm font-bold text-gray-900 truncate font-outfit">{app.firstName} {app.lastName}</span>
                                                <span className="text-xs text-gray-500 flex items-center gap-1">
                                                    {app.gender === 'MALE' ? 'Male' : app.gender === 'FEMALE' ? 'Female' : app.gender}, {new Date().getFullYear() - new Date(app.dateOfBirth).getFullYear()} years
                                                </span>
                                            </div>
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl border-gray-100">
                                                ...
                                                <DropdownMenuSeparator />
                                                <div className="px-2 py-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Update Status</div>
                                                <DropdownMenuItem onClick={() => handleStatusUpdate(app.id, "APPROVED")} className="flex items-center gap-2 cursor-pointer py-2 text-green-600 font-semibold">
                                                    <div className="w-2 h-2 rounded-full bg-green-500" /> Approve
                                                </DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => handleStatusUpdate(app.id, "REJECTED")} className="flex items-center gap-2 cursor-pointer py-2 text-red-600 font-semibold">
                                                    <div className="w-2 h-2 rounded-full bg-red-500" /> Reject
                                                </DropdownMenuItem>
                                                <DropdownMenuSeparator />
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                            ...
                            <TableRow>
                                <TableCell colSpan={6} className="h-48 text-center bg-gray-50/30">
                                    <div className="flex flex-col items-center justify-center gap-2">
                                        ...
                                        <p className="text-sm font-medium text-gray-500">No applicants found matching your criteria.</p>
                ...
                <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between font-outfit">
                    <p className="text-xs text-gray-500 font-medium tracking-tight">
                        Showing <span className="text-gray-900 font-bold">{filteredApplicants.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}</span> to <span className="text-gray-900 font-bold">{Math.min(currentPage * itemsPerPage, filteredApplicants.length)}</span> of <span className="text-gray-900 font-bold">{filteredApplicants.length}</span> entries
                    </p>
...
const Star = ({ className, fill }: { className?: string, fill?: string }) => (
    <svg className={className} fill={fill || "none"} stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.921-.755 1.688-1.54 1.118l-3.976-2.888a1 1 0 00-1.175 0l-3.976 2.888c-.784.57-1.838-.197-1.539-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.382-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
    </svg>
);
